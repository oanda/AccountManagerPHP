...
class oandaAPI {
    ...
    function retrieveAccount($accountNumber) {
        ...
        $this->homecurr = $result['homecurr'];
    }
    
    // Retrieve information for Open Trades
    function retrieveTrades($accountNumber) { 
    	$ch = curl_init("https://api-sandbox.oanda.com/accounts/".$accountNumber."/trades");
    	curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    	curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
        $result = curl_exec($ch);
        $result = json_decode($result, true);
    ...
    function retrieveUserProfile($username) {
	    $ch = curl_init("https://api-sandbox.oanda.com/accounts?username=".$username);
	    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
	    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
	    $result = curl_exec($ch);
	    $result = json_decode($result, true);
